 ...
 * move backwards ‘M’ indices. You should assume that the array is circular
 ...
 * the Fast & Slow pointer method is an efficient way to do that. We can start
 * from each index of the array to find the cycle. If a number does not have a
 * cycle we will move forward to the next element. There are a couple of additional
 * things we need to take care of:
 * <p>
 * As mentioned in the problem, the cycle should have more than one element.
 * This means that when we move a pointer forward, if the pointer points to the
 * same element after the move, we have a one-element cycle. Therefore,
 * we can finish our cycle search for the current element.
 * <p>
 * The other requirement mentioned in the problem is that the cycle should not
 * contain both forward and backward movements. We will handle this by remembering
 * the direction of each element while searching for the cycle. If the number is positive,
 * the direction will be forward and if the number is negative, the direction will be
 * backward. So whenever we move a pointer forward, if there is a change in the direction,
